...
class MyWin(Ui_MainWindow, QMainWindow):
    ...
    def hari_test_action(self):
        start_result = QMessageBox.information(None, "结果确认", "请确认测试结果", QMessageBox.Yes|QMessageBox.No)
        if start_result == QMessageBox.Yes:
        ...
        if start_result == QMessageBox.No:
            stop_button.MyWin.onPushButtonClick_stop(self)
            total_time.MyWin.Total_Test_time(self)
            testStatusText = 'Fail'
            testresult = 'Fail'
