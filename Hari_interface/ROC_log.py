...
class TestMyLog(object):
     def print_log(self,robot_sn, detail_SN, IMEI):
         ...
         chrome_browser.find_element_by_css_selector("[placeholder='请输入管理员编码']").send_keys(
             "%s" % url_roc_login_user)
         ...
         chrome_browser.find_element_by_xpath(
             ...
             "%s" % robot_sn_internet)
         #####关闭
         #mylogger.info("推送策略子页面填写完毕后点击关闭")
         #chrome_browser.find_element_by_xpath(
         #    "/html/body/div[2]/div[2]/div/nz-modal/div/div[2]/div/div/div/app-add-push/form/div/button[1]").click()
         time.sleep(0.5)
         #####确认保存
         mylogger.info("推送策略子页面填写完毕后点击应用")
         chrome_browser.find_element_by_xpath(
             "/html/body/div[2]/div[2]/div/nz-modal/div/div[2]/div/div/div/app-add-push/form/div/button[2]").click()
         time.sleep(2)
         chrome_browser.quit()
