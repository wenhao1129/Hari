    ...
    def Self_logfile(self, testresult):
        ...
        class sourcePath_case(object):
            ...
            def case_fun_Unit(self, sourcePath):
                ...
                sourcePath = '%s/unit.txt' % unitName
                return sourcePath
        ...
        class targetPath_case(object):
            ...
            def case_fun_device(self, localtargetPath):
                localtargetPath = '%s/%s/%s/%s/%s.txt' % (
                locallogpath, modelName, logfileSaveDate, stationName, sntime)
                return localtargetPath
            ...
            def case_fun_write_document(self, localtargetPath):
                localtargetPath = '%s/%s/%s/%s/%s.txt' % (
                locallogpath, modelName, logfileSaveDate, stationName, sntime)
                return localtargetPath
        ...
        class ServertargetPath_case(object):
            ...
            def case_fun_device(self, serverTargetPath):
                serverTargetPath = '%s/%s/%s/%s/%s.txt' % (serverPath, modelName, logfileSaveDate, stationName, sntime)
                return serverTargetPath

            def case_fun_write_document(self, serverTargetPath):
                serverTargetPath = '%s/%s/%s/%s/%s.txt' % (serverPath, modelName, logfileSaveDate, stationName, sntime)
                return serverTargetPath
            ...
            def case_fun_compare_version(self, serverTargetPath):
                serverTargetPath = '%s/%s/%s/%s/%s.txt' % (serverPath, modelName, logfileSaveDate, stationName, sntime)
                return serverTargetPath
        ...
        if not os.path.exists(logfilesourcePath):
           self.textBrowser.append('没有找到logfile, 请联系TE')
           self.lineEdit_teststatus.setText('Fail')

        else:
            try:
                shutil.copyfile(logfilesourcePath, serverTargetPath)
            except (IOError, NameError, FileNotFoundError) as exceptresult1:
